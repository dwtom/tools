 ...
 * @Description: 压缩图片
 * @Author: Dong Wei
 * @Date: 2020-01-15 09:15:53
 * @LastEditors: Dong Wei
 * @LastEditTime: 2023-04-07 16:36:12
 */
class Compress {
  // 处理异常情况
  handleError(file, allowSize, callback) {
    // 判断图片格式
    ...
    // 判断阈值
    if (typeof allowSize !== "number" || allowSize === NaN || allowSize <= 0) {
      return callback({
        success: false,
        msg: "请传入正确的阈值",
        file: "",
    ...
    // 避免部分安卓手机点击上传但不传图片直接取消也能走下去
    if (!file.size) {
      return;
    }
  }
  /**
   * @description: canvas压缩流程 (默认压缩率30%)
   ...
   * @return: {void}
   */
  canvasDataURL(path, config, callback) {
    let img = new Image();
    img.src = path;
    img.onload = function () {
      let that = this;
   ...
   * @param {Number}[allowSize=2] 处理压缩的阈值(默认2MB),图片大小小于阈值则不压缩
   * @param {Function}[callback] 回调函数 传出对象 success-是否有返回值,msg-错误信息,file-文件对象
   * @return: 回调函数
   */
  handleResult(event, config, allowSize = 2, callback) {
    let file = event.target.files[0] || "";
    this.handleError(file, allowSize, callback);
    ...
    if (file.size / 1024 > 1024 * allowSize) {
      ...
      ready.onload = function () {
        ...
        // 压缩后的base64
        that.canvasDataURL(path, config, (base64Url) => {
          ...
          if (size < allowSize) {
            ...
            let compressFile = that.convertBase64UrlToFile(base64Url, file.name);
            return callback({
              success: true,
              msg: "",
              file: compressFile,
